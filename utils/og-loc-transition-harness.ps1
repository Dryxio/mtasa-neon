...
public static class NeonPhysicalInput
{
...
function Invoke-PhysicalKey
{
    ...
    [NeonPhysicalInput]::KeyDown($ScanCode)
    try
    {
    ...
    {
        [NeonPhysicalInput]::KeyUpEvent($ScanCode)
    }
}
...
Write-Host "[og-loc-transition-runner] Watching server and client logs"
Write-Host "[og-loc-transition-runner] GTA PIDs: $(@($clients.Id) -join ', ')"
...
while (((Get-Date) - $startedAt).TotalSeconds -lt $TimeoutSeconds)
{
    foreach ($log in $logs)
    {
        ...
        {
            if ($line -match '\[og-loc-transition-jsonl\]\s+(\[.*\])')
            {
                $records = @()
                try
                {
                ...
                foreach ($record in $records)
                {
                    ...
                    Write-Host "[og-loc-transition-runner] probe=$probeKey PID=$($client.Id) raw-W then awaiting-ui-W"
            ...
            if ($log -eq $ServerLog)
            {
                if ($line -match '\[og-loc-transition\]\s+PASS\s+profile=([^\s]+)')
                {
                    Write-Host "[og-loc-transition-runner] PASS profile=$($Matches[1])"
                    exit 0
                }
                if ($line -match '\[og-loc-transition\]\s+FAIL\s+profile=([^\s]+)(.*)$')
                {
                    Write-Error "[og-loc-transition-runner] FAIL profile=$($Matches[1])$($Matches[2])"
                    exit 1
                }
            }
        }
...
Write-Error "[og-loc-transition-runner] FAIL: no terminal verdict within $TimeoutSeconds seconds"
exit 2
