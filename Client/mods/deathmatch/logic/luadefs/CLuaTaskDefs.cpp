...
void CLuaTaskDefs::LoadFunctions()
{
    ...
    CLuaCFunctions::AddFunction("setPedPhysicalProofs", ArgumentParser<SetPedPhysicalProofs>);
...
bool CLuaTaskDefs::SetPedPhysicalProofs(CClientPed* ped, bool bullet, bool fire, bool explosion, bool collision, bool melee)
{
    if (!ped || ped->GetType() != CCLIENTPED || !ped->IsStreamedIn() || !ped->GetGamePlayer() ||
        (!ped->IsLocalEntity() && !ped->IsSyncing()))
    {
        return false;
    }

    ped->GetGamePlayer()->SetPhysicalProofs({bullet, fire, explosion, collision, melee});
    return true;
}
...
int CLuaTaskDefs::setPlayerTask(lua_State* luaVM)
{
    ...
    if (argStream.NextIsTable() && pEntity && pEntity->GetType() == CCLIENTPLAYER)
    {
