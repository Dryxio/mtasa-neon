...
public:
    ...
    // Non-null only while an authenticated server-forwarded hit is being
    // replayed synchronously. DamageHandler uses the exact attacker/victim
    // pair to distinguish it from damage generated by a remote presentation.
    CClientPed*          m_pNativeDamageReplayVictim{};
