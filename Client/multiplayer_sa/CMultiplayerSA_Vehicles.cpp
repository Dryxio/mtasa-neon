...
static void __fastcall MonsterTruckHydraulicControl(CAutomobileSAInterface* vehicle)
{
    ...
    // Neon relocates the model table when extending file IDs. Use its model lookup.
    auto* model = pGameInterface->GetModelInfo(vehicle->m_nModelIndex);
    auto* modelInfo = model ? model->GetInterface() : nullptr;
    if (!modelInfo)
        return;
