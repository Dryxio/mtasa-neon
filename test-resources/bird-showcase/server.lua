...
local function restore()
    ...
    if isElement(player) then
        triggerClientEvent(player, "birdShowcase:stop", resourceRoot)
        if saved then
...
local function start(player)
    if state.running then
        outputChatBox("[BIRD SHOWCASE] Another showcase is already running.", player, 255, 90, 90)
        return
    end
    ...
    setElementPosition(player, CENTER_X, CENTER_Y - 45, 16.8)
    setElementAlpha(player, 0)
    setElementFrozen(player, true)

    setTimer(function()
        if state.running and state.player == player and isElement(player) then
            triggerClientEvent(player, "birdShowcase:start", resourceRoot, CENTER_X, CENTER_Y, CENTER_Z, SHOW_DIMENSION)
        end
    end, 600, 1)
end

addCommandHandler("birdshow", function(player, _, action)
    action = (action or "start"):lower()
    if action == "stop" or action == "reset" then
        if state.player == player then restore() end
        return
    end
    if action == "shoot" then
        triggerClientEvent(player, "birdShowcase:shoot", resourceRoot)
        return
    end
    if action ~= "start" then
        outputChatBox("[BIRD SHOWCASE] Usage: /birdshow [start|stop|shoot]", player, 255, 200, 80)
        return
    end
    start(player)
end)

addEvent("birdShowcase:finished", true)
addEventHandler("birdShowcase:finished", resourceRoot, function()
    if client and client == state.player then restore() end
end)
