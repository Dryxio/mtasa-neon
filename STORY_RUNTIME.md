...
Mission-passed music uses a separate verified path. Opcode `0394` reaches handler `0x4842AF`, accepts selectors `1` and `2`, adds `10`, calls `CAudioEngine::PreloadBeatTrack` at `0x507F40`, then calls `PlayPreloadedBeatTrack(true)` at `0x507180`. `playMissionPassedTune` exposes only those two selectors and delegates to Neon's existing `PlayBeatTrack` wrapper, which performs the same pair of native calls. `Tagging Up Turf` uses selector `1` and awards the visible SCM amount of `$200`; manual `/tagupfinal` validation confirmed both effects after the `17546 ms` Grove finale. Campaign progression, respect, gang-zone, wanted-level, statistics, pickup, and unlock bookkeeping remain intentionally outside the current multiplayer conformance scope.
