...
> **Important release compatibility:** exact synchronization above `2047.5` HP is intentionally deferred while public clients and servers remain on protocol `1E6`. Commit `890fa5688` implemented an exact-float escape, but also changed the vehicle bitstream and therefore required a coordinated client/server protocol bump. It was reverted for the client-only Build 185. Restore that complete commit only for a coordinated client and server release, bump the protocol, and rerun its two-client health harness.
...
The 30 July two-client pass added a transversal weapon-presentation channel for non-syncers. The authoritative ped exports the active native `FIRE` or `DRIVEBY` shape, live weapon, burst length, shooting rate and target point; drive-by snapshots also carry the native abort range, frequency, style and seat side. A viewer installs an exactly owned GTA presentation task, updates its target in place and removes only that task when the sample changes, expires or loses its native slot. Instant-hit presentation is cancelled before ammo, line-of-sight, projectile, damage or script-fire side effects, then emits only the local point light, gunshot FX, gun flash, shell and weapon audio. Area-effect spray retains GTA's native local FX while the existing presentation guards reject damage and tag progress. Ordinary ped sync remains authoritative for transforms, vehicle health, player health and death, so this correction applies to every mission using the same native weapon-task channel rather than to SWEET3 alone.

In the successful regression, the observer reconstructed Sweet, Ryder and the Ballas passenger's vehicle fire, then both surviving Ballas' on-foot bursts. The syncer alone reduced the Voodoo to `240`, both clients converged on both Ballas dead, stale presentation cleared before `return_grove_drive`, and neither client logged a network or weapon error. The observer's aim followed the correct Greenwood/CJ target rather than its local camera, but coordinate snapshots are not frame-identical entity tracking: during fast vehicle motion one sample lagged the visible target by roughly `12 m` and a brief heading sample differed by about `37` degrees before the next update. Gameplay damage still arrived from the authoritative syncer. One earlier attempt timed out the restaurant reconstruction barrier at `stable=0/3`; an immediate retry passed it in two seconds and the timeout occurred before weapon presentation activated, so it remains a separate intermittent harness condition to monitor.
...
Manual validation confirmed CJ's fresh-game world appearance in `/tagupfinal`, correct native `CSPLAY` rendering in `/tagupsweet1a`, and the transition into the world intro. The 30 July 2026 complete two-client run then confirmed that only the leader becomes CJ, the companion retains its original skin, both clients observe the synchronized CJ appearance, and successful cleanup restores the leader's saved model and clothes.
