...
- Execute each native AI task on one syncer only. Non-syncers may reproduce the final locomotion, pose, animation, aim, and interaction state, but must not run a competing task or produce gameplay side effects.
- Treat server task ownership and remote presentation as separate contracts. The server reconstructs semantic work after an owner epoch changes; observers consume generation-scoped baselines and snapshots rather than raw `CTask` memory.
